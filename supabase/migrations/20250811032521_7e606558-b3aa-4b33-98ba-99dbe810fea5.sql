...
create policy if not exists "Users can view their own wins"
  on public.wins for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own wins"
  on public.wins for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own wins"
  on public.wins for update
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own wins"
